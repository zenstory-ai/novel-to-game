...
- `tests/fixtures/minimal-evidence/` 用无依赖的 CPython 程序跑出一份合规记录，证明契约不预设浏览器；`browser` 只是 web 目标的运行时通道字段。

来源：91351f1、bb3aa64、45d2c3c (#15)、6c450da (#20)、edbb962 (#28)、58e0ae4 (#44)
...
- **smoke / delivery / release 三档 assurance profile（45d2c3c 落地一天）** — 最强理由：不同成色需要不同强度。否决原因：等级表把"完成"变成可协商的层级，#20 收成一条路径，成色改由 `targetFinish` 表达。
