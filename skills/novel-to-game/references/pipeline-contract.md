...
| 阶段 | 交接前必须成立 |
|---|---|
| 需求 intake | 十一个产品维度（平台+性能预算/市场/类型+对标/画风/分级/核心幻想/时长/发行/游戏引擎/玩家结构+社交/受众画像）各有取值，用户确认或未确认假设已醒目记录；`PRODUCT_BRIEF.md` 存在 |
| 拆解 | 全书/指定范围覆盖集合完整且无失败缺口；核心原作事实有证据，玩家动作、空间、角色和独特锚点明确 |
| 概念 | 三个方案真正不同，无硬否决，选择明确 |
| 游戏设计 | 核心循环、世界响应、关卡节奏、范围和结果完整 |
| 美术方向 | 视觉语言、可读反馈和每个交互界面 / 模式各有招牌时刻（不固定三张）明确 |
| 构建 | 可运行路径存在并实际操作过 |
| 质量验证 | 零 `blocker`/`major`，核心动作、结果和重开有证据；`QA_REPORT.md` 已落盘（**阻断交付物**，无它不得报完成），记录三项主观-但-可观察裁决：首次上手 / 核心幻想演出 / 招牌帧符合 |

多人、持久后端、资产流水线和额外章节，凡未在 `PRODUCT_BRIEF` 的玩家结构 / 社交形态里
显式确认过的，都属于新的范围决定，不得静默加入；已确认为成品核心幻想一部分的社交 / 联机，
其在切片里的表现假设按 intake 锁定来做（原型仍是单人网页切片）。平台、类型、画风、分级、
玩家结构、受众画像、性能预算不是范围决定而是 intake 已锁定的产品事实，下游要改同样回
`PRODUCT_BRIEF.md` 显式改。

## 上游事实不得在下游静默改写

有两份上游事实：`PRODUCT_BRIEF.md`（产品框架：平台+性能预算、市场、类型+对标、画风、
分级、核心幻想、玩家结构 / 社交、受众画像）与 `SOURCE_BIBLE.md`（原作事实）。两者都常在下游被**悄悄替换成更好用的版本**。
