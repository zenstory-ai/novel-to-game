...
产品框架（平台、目标市场、主类型方向、画风、分级、核心幻想、时长、**受众画像**、**玩家结构 /
社交形态**）已由 `PRODUCT_BRIEF.md` 在 intake 阶段锁定，**直接继承，不重猜、不静默改**。
受众画像（硬核度 / 性别向 / 玩家动机原型）是决策密度、UI 密度、单局时长与教学强度的标尺，
本阶段继承并落地，不在概念里另起一套。本阶段只在这个框架内把它落成一页可指导
...
常已过时或有误。联网不可用时，把对标事实明确标注为「凭记忆·未核实」，不当作确定结论。
...
- 核心动词、循环、压力和熟练度差异；
- 它如何把原作的规则**或情感 / 伦理张力**变成玩家行动——**哪一个可重复核心动词让玩家亲手
  做出这份幻想**（禁用 finisher 脚本 / 一次性道具代劳）；
- 最关键的玩法对标，以及只借其中哪条原则；
- 一张最能传播且能看出玩法的画面；
