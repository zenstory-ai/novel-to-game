...
的可玩证据和一个会否决它的失败现象。若发现 `PRODUCT_BRIEF` 的某项与原作适配明显冲突，
回总入口显式修订，不在本阶段擅自更改。
...
说法（对照表、可用先例名单与分支结构清单见 concept-method.md「互动叙事这条线」）。反过来，选了叙事主导
也不得省略结构选型，选一个并说明理由；hybrid 的叙事层同样要选。

对标组合必须同时覆盖玩法问题与文化市场问题（一款游戏可同时承担两种证据，不为地域凑名单）；区分可迁移的玩法原则与不可照搬的文化符号、笑点、价值关系和商业惯例。
...
- 最小验证切片（默认 10-30 分钟，且不超过 `PRODUCT_BRIEF` 锁定的单局时长）证明什么、
  ...
  切片与全量的范围差、全量何时才做；
- 最小验证问题：只做哪一段可玩内容，就能在可玩验证中证伪最大风险。
