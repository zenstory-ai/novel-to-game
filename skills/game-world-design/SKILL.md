...
叙事实际承载体验时读取 [narrative-design-method.md](references/narrative-design-method.md)，
不为所有项目强制写长篇对白。
...
1. **体验与循环合同**：玩家身份、承诺、目标感受、`experienceProfile`、核心动作与循环和主要玩法先例，
   以及原作如何改变动作对象、顺序、代价和世界回应；
2. **世界规则与进程**：玩家可见/隐藏信息、行动顺序、随机性、资源或持久状态及其读取点；
   适合本项目的局面、能力或理解变化，以及可观察结束标记，不强套成长阶段；
...
按 world-design-method 反向走读可执行闭环、世界回响、设计进程、重放与范围；存在叙事或专属命令时，
